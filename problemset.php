...
if (isset($_SESSION['administrator'])){
	...
	$sql="SELECT `problem_id`, `difficulty_level`, `title`,`source`,`submit`,`accepted` FROM `problem` WHERE $filter_sql ";
	
}
else{
	$now=strftime("%Y-%m-%d %H:%M",time());
	$sql="SELECT `problem_id`,`difficulty_level`,`title`,`source`,`submit`,`accepted` FROM `problem` ".
...
while ($row=mysqli_fetch_object($result)){
	...
	$view_problemset[$i][2]="<div class='center'>".$row->difficulty_level."</div>";;
	$view_problemset[$i][3]="<div class='left'><a href='problem.php?id=".$row->problem_id."'>".$row->title."</a></div>";;
	$view_problemset[$i][4]="<div class='center'><nobr>".mb_substr($row->source,0,8,'utf8')."</nobr></div >";
